...
| "Le modele Whisper 'xxx' n'est pas installe sur cette machine" | Le modele n'a pas encore ete telecharge (l'application ne telecharge jamais rien elle-meme, pour ne pas risquer un blocage en pleine reunion) | `python telecharge_modele.py` : telechargement avec reprise automatique sur coupure, relançable autant de fois que necessaire, chaque relance conserve ce qui est deja recu |
| Telechargement du modele Whisper lent ou hache | Reseau instable (transferts qui calent) | C'est prevu : `telecharge_modele.py` detecte un blocage en 30 s max et reprend a l'octet exact ou ca s'est arrete ; laisser tourner, ou relancer plus tard, rien n'est perdu |
